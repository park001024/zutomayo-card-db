...
| | 출처 | 비고 |
|---|---|---|
| 일본어 원문·수치 | 공식 사이트 MeiliSearch API | 425장 전부. 이름·효과·시계·공격력·코스트·팩·수록곡·일러스트레이터 |
| 한국어 번역 | 이 저장소 (`data/i18n/ko.json`) | 카드명 286개 · 곡명 42개 · 효과 231개를 일본어 원문에서 번역 |
| 공식 미공개 카드 | `data/extra_cards.json` (손으로 채움) | 지금은 비어 있다. 4탄 SE 3장(4th_105~107)이 여기 있었으나 2026-08 공식 공개로 넘어갔다 |
| 카드 이미지 | 공식 CDN 원본 425장 | 공식 JPG 원본을 보존용으로 함께 커밋한다. 화면에는 원본과 같은 크기(대개 700×978)의 WebP 와 폭 400 썸네일을 쓴다 (원본 150MB / WebP 81MB) |
...
- **정렬** 탄·번호 / 이름 / 등급 / 크로노스 / 파워코스트 / 밤·낮 공격력.
  기준 왼쪽의 버튼으로 오름차순·내림차순을 전환한다. 동점이면 탄 → 카드 번호 순.
  값이 없는 카드(예: 인챈트의 공격력)는 방향과 무관하게 항상 뒤로 간다
...
data/extra_cards.json      공식에 없는 카드 — official_cards 와 같은 필드 구조 (일본어) ← 손으로 채운다 (현재 비어 있음)
