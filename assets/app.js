 ...
 *
 * 따옴표류(" ' “ ” ‘ ’ 「 」 『 』)는 전부 " 하나로 합친다. 「」 를 " 로도 ' 로도
 * 찾으려면 세 부호가 같은 자리로 모여야 하고, 그러면 큰따옴표·작은따옴표 구분은
 * 유지할 수 없다. 서로 찾게 하는 쪽을 택했다.
 *
 * 합치지 않은 것 — ★☆ 는 채움/비움이 뜻이 다르고, ♡ ※ 는 대응하는 ASCII 가 없다.
 * ・ 는 중점끼리 따로 모으므로 여기 넣지 않는다.
 */
const PUNCT = {
  '\u0027': '"', '\u201c': '"', '\u201d': '"', '\u201e': '"', '\u201f': '"',
  '\u2018': '"', '\u2019': '"', '\u201a': '"', '\u201b': '"',
...
// 문자클래스로 쓰므로 ] \ ^ - 는 이스케이프한다. 안 하면 키를 추가할 때
// [“-。] 처럼 범위로 해석되어 4천 자가 넘게 잡힌다.
// g 플래그가 있으므로 replace 로만 쓴다 — test/exec 는 lastIndex 가 남아 결과가 흔들린다.
const PUNCT_RE = new RegExp(
  '[' + Object.keys(PUNCT).join('').replace(/[\]\\^-]/g, '\\$&') + ']', 'g');

/** 검색 정규화 + 위치 추적.
 *  소문자 · 공백 제거 · 전각 ASCII(！~～)→반각 · 가타카나(전각·반각)→히라가나 ·
 *  중점 표기 통일 · 그 밖의 문장부호 통일(PUNCT).
 *
